...
export type CrmRightDrawerProps = {
    open: boolean;
    title: string;
    onClose: () => void;
    ...
    /** Tailwind z-index class when stacking drawers */
    zIndexClass?: string;
};

/**
 * Shared right-side drawer shell for CRM admin flows.
 */
const CrmRightDrawer: React.FC<CrmRightDrawerProps> = ({
    open,
    title,
    onClose,
    children,
    ...
    ariaLabelledBy = "crm-drawer-title",
    zIndexClass = "z-[1050]",
}) => {
    useEffect(() => {
        if (!open) return;
    ...
    const overlayZ = zIndexClass.replace("1050", "1040").replace("1060", "1055");
    ...
    return (
        <>
            ...
            />
            <aside
                className={`fixed right-0 top-0 h-full w-full ${maxWidthClass} bg-white dark:bg-bodybg shadow-xl ${zIndexClass} flex flex-col animate-slide-in-right`}
                role="dialog"
                aria-modal="true"
