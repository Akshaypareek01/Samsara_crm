...
const UserProfile = () => {
  ...
  const [periodOverview, setPeriodOverview] =
    useState<AdminPeriodOverview | null>(null);
  ...
  useEffect(() => {
    if (activeTab === "period" && userId && !periodOverview) {
      fetchPeriodData();
    }
  }, [activeTab, userId]);
  ...
  const fetchUserData = async () => {
    try {
      ...
      const [userData, dashboard, waterToday, hydration] =
      ...
      if (dashboard.status === "fulfilled") {
        ...
        console.error(
          "❌ Failed to fetch tracker dashboard:",
          dashboard.reason
        );
      }
      ...
      if (waterToday.status === "fulfilled") {
        ...
        console.error(
          "❌ Failed to fetch today water data:",
          waterToday.reason
        );
      }
  ...
  const fetchPeriodData = async () => {
    try {
      ...
      const [overviewRes, cyclesRes] = await Promise.allSettled([
        AdminPeriodService.getUserPeriodOverview(userId),
        AdminPeriodService.getUserPeriodCycles(userId),
      ]);

      if (overviewRes.status === "fulfilled") {
        setPeriodOverview(overviewRes.value);
      }

      if (cyclesRes.status === "fulfilled") {
        setPeriodCycles(cyclesRes.value || []);
      }
    } catch (err) {
      console.error("❌ Failed to fetch period data:", err);
      setPeriodError("Failed to load period data");
    } finally {
                ...
                <div className="flex-grow main-profile-info">
                  <div className="flex items-center !justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        <i className="ri-arrow-left-line me-1 align-middle inline-block"></i>
                        Back
                      </Link>
                    </div>
                  </div>
                  ...
                  <div className="mb-0">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {user.goal.map((goal, idx) => (
                        <span
                          key={idx}
                          className="badge bg-primary/10 text-primary m-1"
                        >
                          {goal}
                        </span>
                      ))}
                    ...
                    >
                      <button
                        className={`w-full sm:w-auto flex ${
                          activeTab === "overview"
                            ? "active hs-tab-active:font-semibold hs-tab-active:text-white hs-tab-active:bg-primary"
                            : ""
                        } rounded-md py-2 px-4 text-primary text-sm`}
                        onClick={() => setActiveTab("overview")}
                      >
                        <i className="ri-dashboard-line align-middle inline-block me-1"></i>
                        Overview
                      </button>
                      <button
                        className={`w-full sm:w-auto flex ${
                      ...
                      >
                        <i className="ri-heart-line me-1 align-middle inline-block"></i>
                        Health
                      </button>
                      <button
                        className={`w-full sm:w-auto flex ${
                      ...
                      >
                        <i className="ri-emotion-line me-1 align-middle inline-block"></i>
                        Mood
                      </button>
                      <button
                        className={`w-full sm:w-auto flex ${
                  ...
                  <div className="!p-4">
                    ...
                    {activeTab === "overview" && (
                      <div className="grid grid-cols-12 gap-4">
                            ...
                            <div className="box-body">
                              <div className="space-y-4">
                                ...
                                <div className="flex justify-between items-center">
                                  ...
                                  <span
                                    className={`font-semibold ${getHydrationColor(
                                ...
                                {hydrationStatus && (
                                  <div>
                                    ...
                                    <div className="w-full bg-gray-200 rounded-full h-2">
                                      <div
                                        className={`h-2 rounded-full ${getHydrationBgColor(
                                          hydrationStatus.status
                                        )}`}
                                        style={{
                                          width: `${Math.min(
                        ...
                        {todayWater &&
                          todayWater.intakeTimeline &&
                          todayWater.intakeTimeline.length > 0 && (
                            <div className="xl:col-span-12 col-span-12">
                              <div className="box">
                                <div className="box-header">
                                  <h5 className="box-title">
                                    Today's Water Intake Timeline
                                  </h5>
                                </div>
                                <div className="box-body">
                                  <div className="space-y-2">
                                    {todayWater.intakeTimeline.map(
                                      (intake, idx) => (
                                        <div
                                          key={idx}
                              ...
                              {trackerData?.weight ? (
                                <div className="text-center">
                                  <p className="text-3xl font-bold text-primary mb-2">
                                    {trackerData.weight.currentWeight?.value ||
                                      "-"}{" "}
                                    {trackerData.weight.currentWeight?.unit ||
                                      "kg"}
                                  </p>
                              ...
                              {hydrationStatus && (
                                <div className="mt-4">
                                  <div className="flex justify-between text-sm mb-2">
                                    ...
                                    <span
                                      className={`font-medium ${getHydrationColor(
                                        hydrationStatus.status
                                      )}`}
                                    >
                                      {hydrationStatus.status} (
                                  ...
                                  <div className="w-full bg-gray-200 rounded-full h-2.5">
                                    <div
                                      className={`h-2.5 rounded-full ${getHydrationBgColor(
                                        hydrationStatus.status
                                      )}`}
                                      style={{
                                        width: `${Math.min(
                                  ...
                                  <div className="mt-3 text-sm text-muted">
                                    <p>
                                      Remaining: {hydrationStatus.remainingMl}ml
                                      ({hydrationStatus.remainingGlasses}{" "}
                                      glasses)
                                    </p>
                                  </div>
                                </div>
                    ...
                    {activeTab === "period" && (
                      <div className="box">
                        ...
                        <div className="box-body">
                          ...
                          {!periodLoading && periodOverview?.hasData && (
                            <div className="space-y-4">
                              {/* Summary */}
                              <div className="grid grid-cols-3 gap-4 text-center">
                                <div>
                                ...
                                <div>
                                  <p className="text-2xl font-bold text-info">
                                    {periodOverview.lastPeriodStart
                                      ? new Date(
                                          periodOverview.lastPeriodStart
                                        ).toLocaleDateString()
                                      : "—"}
                              ...
                              <div className="mt-6">
                                ...
                                {!periodCycles || periodCycles.length === 0 ? (
                                  <p className="text-muted">
                                    No period cycles recorded
                                  </p>
                                ) : (
                                  <div className="space-y-2">
